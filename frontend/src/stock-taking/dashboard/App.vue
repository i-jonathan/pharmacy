  ...
  <div class="max-w-6xl mx-auto px-4 py-8">
        ...
        <table class="w-full text-sm text-left">
          ...
          <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
            ...
            >
              ...
              <td class="px-6 py-4 text-gray-600 dark:text-gray-300">
                {{ st.completed_at ? formatDate(st.completed_at) : "—" }}
              </td>
              <td class="px-6 py-4 text-gray-600 dark:text-gray-300">
                {{ st.completed_by || "—" }}
              </td>
    ...
    >
      ...
      >
        ...
        <div v-if="createError" class="mb-4 p-3 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-700 rounded-lg text-red-700 dark:text-red-300 text-sm">
          {{ createError }}
        </div>
  ...
  methods: {
    async fetchStockTakings() {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          throw new Error(err.message || "Failed to load stock takings");
        }
    ...
    async handleCreate() {
      ...
      try {
        ...
        if (!res.ok) {
          throw new Error(data.message || "Failed to create stock taking");
        }
